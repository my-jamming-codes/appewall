---
heroImage: /src/assets/images/go.webp
category: Go
...
1. What is the special package in Go that tells the Go compiler this is the main package for your program?

```html
a) main b) pkg c) cli d) init
```

2. Which function must the main package contain?

```html
a) init b) start c) main d) run
```

3. What is the purpose of the blank identifier in Go?

```html
a) To store values you dont need. b) To create global variables. c) To initialize the program. d) To
terminate the program.
```

4. How is the blank identifier represented in Go?

```html
a) * b) # c) _ d) &
```

5. When is the init function called in Go?

```html
a) When the program starts. b) When a package is imported. c) After the main function. d) When the
program ends.
```

6. Can you have multiple init functions in a single Go package?

```html
a) Yes b) No
```

7. What prefix is used to import a package for its side effects only?
...
```astro
1. a) main 2. c) main 3. a) To store values you dont need. 4. c) _ 5. b) When a package is imported.
6. a) Yes 7. c) _
```
